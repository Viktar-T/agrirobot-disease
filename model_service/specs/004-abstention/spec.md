...
**Branch**: `004-abstention` · **Created**: 2026-09-20 · **Status**: Specified 2026-09-20 with failing tests in `model_service/tests/test_abstain.py` (50 red, 4 green); `ms.abstain` and the rows it adds land with the W4 "Scores" and "N3" tasks (DECISIONS 86–93)
...
1. **Given** the validation slice's scores and a declared coverage `c` ∈ {0.80, 0.90, 0.95} (FR-012 names them; 0.90 is the default operating point), **then** each threshold is the empirical quantile that keeps `c` of the slice's rows by **that score alone**:
   - `tau_conf` is the (1 − c) quantile of `conf`, and a row abstains when `conf < tau_conf`;
   - `tau_knn` is the c quantile of `knn`, and a row abstains when `knn > tau_knn`;
   - `tau_maha` is the c quantile of `maha`, recorded and reported, never part of the decision (US-4.3).

   Quantiles are linear-interpolated (`numpy.quantile`'s default), so a threshold need not be one of the observed values. At `c = 0.95` the distance rule is H8's "threshold at TPR 95 % on in-domain validation".
2. **Given** the three thresholds of one coverage, **then** `abstain.json` also records `coverage_achieved`: the share of the validation slice that the **joint** rule of US-4 keeps. It is at most `c`, because two rejection sets are united, and it is the honest number to read beside the declared one.
...
- **FR-005 Thresholds.** As US-3: per declared coverage, the empirical quantile of the validation slice for each score, with `coverage_achieved` for the joint rule. The declared coverages are the config's, and `{0.80, 0.90, 0.95}` is the default set.
...
- **FR-012 Config.** `model_service/configs/abstain.yaml`, one file, with `k` (20), `coverages` (`[0.80, 0.90, 0.95]`), `default_coverage` (0.90), `ece_bins` (15), `epsilon` (1e-6) and `n3`: the held-out sources, each with `manifest` and `classes`, in the shape `configs/eval.yaml` uses for `n2`. Its sha256 is an input (FR-003), so a changed file is a new fit.
...
- **FR-014 Compute log.** A fit that writes appends one row through `ms.compute_log`: `step = "fit_abstain"`, the backbone, resolution and dataset of the run, `n_images_or_runs = 1`, its own `wallclock_s`, `device`, and an `abstain` object with `run_id`, `k`, `n_train`, `n_val` and `T`. `ms.compute_log.STEPS` gains `fit_abstain`, as it gained `env` and `train_head`; every other step keeps its meaning. A fit that was already there writes no row (N7, spec 003 US-6).
...
3. **A declared coverage is per score, on the validation slice, and all three thresholds move with it.** H8 sets τ_conf "for a declared coverage (default 90 %; also report 80 % and 95 %)" and the distance threshold "at TPR 95 % on in-domain validation": read literally, only τ_conf moves and the distance threshold is fixed at the 0.95 quantile. This spec takes the work plan's S4.4 row instead — "thresholds `{tau_conf, tau_knn, tau_maha}` at declared coverages {0.80, 0.90, 0.95}" — so that an operating point is one rule and not two, and at c = 0.95 the two readings coincide.
   - **What differs.** Under H8's literal rule the distance score rejects 5 % of the slice at every coverage; here it rejects 20 % at 0.80 and 10 % at 0.90. Every row at 0.80 and 0.90 — `unknown_recall`, `selective_risk`, `abstention_rate` and the per-class rates — is a different number under the two readings, and Clarification 11 means they are written once. **This one is worth the owner's five minutes before the W4 "Scores" task runs.**
   - Not taken: searching a pair of thresholds whose union rejects exactly 1 − c. It would make each score's threshold depend on the other and leave "TPR 95 %" with no meaning at all.
