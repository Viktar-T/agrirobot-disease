...
- **FR-013 API.** `ms.abstain` exports `SCORES` (`conf`, `knn`, `maha`, `energy`), `DECISION_SCORES` (`conf`, `knn`), `REASONS` (`low_confidence`, `far_from_training`), `COVERAGES`, `Fit`, `fit_run`, `load_fit`, `scores`, `decide`, `probabilities`, `ece` and `auroc`. `ms.abstain.fit` exports `main(argv) -> exit code`.
  ...
  - `scores(fit, run, x)` returns a dict of the four arrays; `decide(fit, scores, coverage)` returns `(decision, abstain_reason)`; `decisions(fit, scores, coverage)` the same for a whole array; `probabilities(fit, run, x)` the temperature-scaled probabilities; `ece(proba, y, bins)` and `auroc(strangeness_known, strangeness_unknown)` the two statistics, so that the eval and the service compute them the same way.
  - The four scores split into the two that come from the head's logits (`logit_scores`) and the two that come from the training features (`distance_scores`); `bank_key(fit)` is what two fits share when those two are the same. The bank, the Gaussians and a target's distances therefore cost one run of a group, not fifteen — which is what makes `make eval` over 181 runs a matter of minutes.
