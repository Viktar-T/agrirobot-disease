...
`python -m ms.eval.run` (`make eval`) scores head runs into rows, aggregates the seeds and renders `n_table.md`; W4 adds `verdict.md`. `python -m ms.eval.probe` (`make probe`) writes the N4 rows. Both write rows only through `ms.eval.append_rows`.
...
1. **Given** a head run (spec 003), **when** `make eval` runs, **then** it scores the test split of the run's own training manifest (`purpose = evaluate`) and appends these rows:
...
- **FR-001 Fields**, in this order (DECISIONS 36): `ts`, `number`, `metric`, `value`, `ci_low`, `ci_high`, `n`, `backbone_id`, `res`, `token_type`, `head`, `seed`, `train_manifest`, `train_manifest_sha256`, `train_split`, `test_manifest`, `test_manifest_sha256`, `test_split`, `split_rule`, `coverage`, `classes`, `run_id`, `model_version`, `cache_key`, `quotable`, `notes`.
  ...
  - `test_split` is `test`, `holdout_unknown` or `cv<k>`: no number is scored on a train or validation split.
...
7. **N2's rows** are settled with N2: the crop-level Makerere direction, which has no healthy crops (DECISIONS 46), and the anthracnose rule (spec 003 Clarification 11).
