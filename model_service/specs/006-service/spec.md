...
- **FR-012 Config.** `model_service/configs/service.yaml`: the served run (US-10.1), `default_coverage`, `max_batch` (32, the backbone configs' batch size), `device` (`auto`), `heads_root`, `cache_root`, `data_root`, `backbone_configs` (the directory US-4.2 reads the backbone's recipe from), `head_configs` (the directory US-10.1 reads the head's recipe from) and `request_log`. Every value is overridable by the environment variables the slice already reads (`MS_HEAD_RUN`, `MS_HEADS_ROOT`, `MS_CACHE_ROOT`, `MS_DATA_ROOT`), and `MS_SERVICE_CONFIG` names the file.
