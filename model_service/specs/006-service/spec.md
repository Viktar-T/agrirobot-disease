...
**Branch**: `006-service` · **Created**: 2026-09-20 · **Status**: Specified 2026-09-20, ahead of W5, with `model_service/tests/test_service.py` at 30 red and 19 green; implemented the same day with the W5 "Service" task (`ms.service.app`, `ms.service.log`, `configs/service.yaml`), 48 of the 49 green. What is left red is US-11, N6, which comes with the W5 "N6" task. US-10.1 gained `head_config_sha256` and US-4.2 the replay-only deployment while the code was written (DECISIONS 120–125)
...
3. **Given** `options`, **then** its keys are `coverage_target` (a number; absent or null means the operating point in force, which is `configs/service.yaml`'s `default_coverage` and, failing that, the fit's) and `return_patch_map` (a boolean, default false). An unknown option is a 422; `return_patch_map: true` is accepted with a warning and changes nothing, because v0 has no localisation (H8 §6.1).
...
1. **Given** a valid request, **then** the answer holds every field of US-1's response block, plus `frame_uid` echoed from the request (DECISIONS 31) and `features` (US-4.3). `scores` has one entry per trained class of the served run and no other, and they sum to 1 within float tolerance. The response is a **closed** shape: FR-003 fixes its fields and their order as a documented superset of H8's example, one function builds it, and a test asserts the list. It is closed by this spec and by that test, **not** by a generated schema: v0 publishes the four endpoints' OpenAPI without request or response models, so the document describes the routes and not the payloads. Declaring the payloads to FastAPI is worth doing when a consumer reads the document; until then the contract lives here (DECISIONS 131).
...
5. **Given** a condition worth naming — `metadata.bbch` is null, `return_patch_map` was asked for, the served head is not quotable, the backbone is research-only until H6 C5 (DECISIONS 13), the bytes are not the size `frame.width` and `frame.height` claim (the computed path alone can see that; a replayed frame is never decoded) — **then** it is a string in `warnings`, and **no warning ever changes `decision`, `scores` or `abstain_reason`** (H8 §6.2). A test asserts that the same frame answered with and without a warning-raising field gives the same decision and the same scores.
...
3. **Given** either path, **then** `features` in the response says which was taken, `"cache"` or `"computed"`.
   - **Given a deployment with no backbone config for the run's backbone**, **then** the service is **replay-only**: it answers cached frames and gives 501 to the rest, saying so, and `GET /v1/model` reports `features: "cache"` instead of `"cache+computed"`. A config that *is* there and whose weights do not resolve is `missing_weights` at start-up, not a surprise on the first uncached frame.
...
2. **Given** a batch whose envelope is valid, **then** the HTTP status is 200 even when some items are invalid: a replayed mission must not lose 500 good frames to one broken one. An item's own `decision`, `abstain_reason = "invalid_input"` and `errors` carry its failure, and a batch-level `n_invalid` counts **those** — the items that broke the contract. A 501 is a frame this deployment cannot answer rather than a bad request, so it is not one of them.
...
1. **Given** `model_service/configs/service.yaml`, **then** it names the served run: `backbone_id`, `res`, `tokens`, `head`, `seed` and the training manifests, or a `run_id` outright, plus the default coverage and the roots. The service resolves it to exactly one run under the heads root.
   - A head's **recipe** is part of that name. `head_config_sha256` is the config's when it gives one, and otherwise the sha256 of `configs/heads/<head>.yaml` as the repository holds it now, so "linear" means the linear recipe that is current and never one the owner retired (DECISIONS 80). A recipe that changed without a retraining resolves to no run, and the service says so rather than serving a superseded one.
...
- **FR-004 Error response.** HTTP 422 with `request_id`, `model_version`, `decision = "abstain"`, `abstain_reason = "invalid_input"`, `errors` (a sorted list of `{path, message}`) and `timing_ms`. HTTP 501 is kept for what the contract allows and this deployment cannot answer — a `uri` scheme pieces 1 and 2 have not fixed yet (DECISIONS 37), and an uncached frame on a replay-only deployment (US-4.2). Its payload is `request_id`, `frame_uid`, `model_version`, `detail` and `timing_ms`: a 501 is a response too, and its log row reads like any other.
...
- **FR-012 Config.** `model_service/configs/service.yaml`: the served run (US-10.1), `default_coverage`, `max_batch` (32, the backbone configs' batch size), `device` (`auto`), `heads_root`, `cache_root`, `data_root`, `backbone_configs` (the directory US-4.2 reads the backbone's recipe from), `head_configs` (the directory US-10.1 reads the head's recipe from) and `request_log`. `MS_SERVICE_CONFIG` names the file, and `MS_HEAD_RUN`, `MS_HEADS_ROOT`, `MS_CACHE_ROOT`, `MS_DATA_ROOT`, `MS_BACKBONE_CONFIGS`, `MS_HEAD_CONFIGS`, `MS_REQUEST_LOG` and `MS_DEVICE` override what it says. A relative path in the environment is resolved against the repository, as one in the file is, and a `MS_SERVICE_CONFIG` that names no file is `bad_config` rather than a silent fall-back.
- **FR-013 Checks at start-up.** Before the service answers anything: the config parses; it resolves to exactly one run; the run loads and its manifests are unchanged; its `abstain.json` loads and matches the run (spec 004 FR-013's `inputs_sha256` check); the cache of its key exists; the default coverage is one the fit carries; the data root exists; the request log's folder can be made; and, when the deployment has a backbone config for the run's backbone, its weights resolve (US-4.2). A failure stops start-up with `<reason>: …`. The reasons are `bad_config`, `missing_run`, `ambiguous_run`, `missing_abstain`, `missing_cache`, `frozen_manifest_modified`, `missing_weights` and `bad_value`.
...
3. **The served model is a config entry, not "the newest run"** (US-10). The slice's rule was fine with one run on disk and is wrong with 361. The verdict names the backbone; the head, the seed and the training manifest are the owner's choice at the "Registry + card" task, and the card records them. **Closed by the owner on 2026-09-20** (DECISIONS 120): the linear head trained on Makerere + iBean, because a demo frame is a frame the head has never seen and that run carries the best N2 the champion has. The verdict settled the backbone; the card records the rest.
...
16. **A deployment without the backbone's own config is replay-only, not broken** (US-4.2). H8 §6.9 makes the cache the reason "the demo does not depend on a GPU", and a central node that holds the caches but not the weights — a machine the DINOv3 licence keeps them off, for one (DECISIONS 13) — can still answer every frame of a replayed mission. It says which mode it is in, and an uncached frame there is a 501 and not a wrong answer. A deployment that *does* configure the backbone must have its weights at start-up.
17. **"Not quotable" is read from the run's `quotable`, not from `--allow-test-only`.** A run that trained on Makerere + iBean is quotable although iBean is test-only (DECISIONS 74), so it raises no warning; a run of the W2 slice's kind is not quotable and raises one (DECISIONS 35).
