...
- [ ] **Interface hand-shake** (M + P, 30 min): confirm with piece 1 that the service request = `observation_frame` + `request_id` + `options`; adopt their answers to the open questions that touch us (`frame_uid` rename; `bbch_null_reason` enum; radians for yaw). Note in `DECISIONS.md`. — *2026-09-19, piece 4's side done (DECISIONS 28–32). From piece 1's draft v0 we adopted the request shape (their choice 1), radians for yaw (choice 4) and the `bbch_null_reason` enum (choice 5), and aligned the frame `sha256` with our content hash. Still open: piece 1 has not reviewed its draft yet (P + E), and `frame_uid` is still their open question; piece 4 supports the rename. Tick once they confirm.*
