...
- [ ] **Verdict** (Fri): `make eval` writes `results/verdict.md` from the N-table by the pre-declared rules (H9 §7: ≥ 2 pp on N2 with non-overlapping five-seed intervals on two of three directions; ≥ 0.02 AUROC on N3; N4 the tie-breaker, N6 reported only; licence = hard gate). No hand edits to the verdict.
