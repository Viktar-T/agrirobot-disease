...
- [ ] **Spec S4.1** (M, 3 h) + `tests/test_manifests.py` failing: field list above; the class map (`class_raw → class_km2`); grouping keys per dataset (Makerere: district → sub-county → date; Tanzania: region/session folders or dates if present, else `phash_group` + `split_rule = "unblocked"`; iBean: test-only, `unblocked`).
- [ ] **Spec S4.2** (M, 2 h) + `tests/test_cache.py` failing: cache key, `meta.json` fields, the two token types, the compute-log row.
- [ ] **Fixture** (M, 1 h): `tests/fixtures/ibean_30/` — 30 iBean images (10 per class) + `LICENSE-MIT` + `manifest.jsonl`; CI runs on CPU on this fixture only.
